...
def main(args):
    ...
    if args.eval_type in ['rgb', 'joint']:
        ...
        rgb_sample = cv2.imread('data/1.jpg')
        # rgb_sample=rgb_sample.reshape(-1,rgb_sample.shape[0], rgb_sample.shape[1],rgb_sample.shape[2])
        rgb_sample=np.tile(rgb_sample,(10,9,1,1,1))
        
        # rgb_sample = tf.convert_to_tensor(rgb_sample)
        # make prediction
    ...
    if args.eval_type in ['flow', 'joint']:
        ...
        flow_sample = np.random.rand(1,9,224,224,2) #np.load(SAMPLE_DATA_PATH['flow'])
